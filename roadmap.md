...
- **✅ DONE (AAA-08)** `core/save.js`: serialize seed + player state + progress; replay-safe. `SaveGame` captures `{ version, seed, player, simClock, progress, meta }` with no wall-clock timestamp; `serialize/fromJSON`, `save/load` (Node-safe shim), deterministic `fingerprint()` (replay-safety), and `restore()` resume are verified in `check_world.mjs` + `save_sim.mjs`. Wired into `main.js` as `window.saveGame` / `window.__worldloop.saveGame` with an init autosave. *(ADR 0022/0023, ADR 0028)* M
...
- **✅ DONE (AAA-10)** Chunk LOD/pooling + streaming budget; remove the double `update()` in `main.js`. `ChunkManager` now exposes a per-frame `streamBudget` + `pendingLoads` queue: missing active chunks are enqueued and drained at most `streamBudget` per `update()`, and LOD rebuilds count against the same budget. Unloaded chunkData is stashed in `chunkPool` (keyed by chunk id) and reused on re-stream (no geometry rebuild). The redundant init-time `chunkManager.update()` in `main.js` is removed — the first per-frame sim update streams the initial world. *(ADR-0030)* L
...
## Phase 15 — 2026 modern test suite (EPIC)

A next-gen game-development test suite, 2026-style: deterministic seeded runs,
property-based fuzzing, golden/snapshot fixtures, headless visual-regression,
perf-regression budgets, mutation testing, and a CI/CD gate — layered on and
