...
A CLI built on the **pi SDK** (`@earendil-works/pi-coding-agent`) that connects to
a **local pi agent** session and drives the **Worldloop** agent loop: it makes the
agent read `AGENTS.md` at the project root and follow its loop strictly — pick the
next `roadmap.md` item, implement it, verify with `check_world.mjs`, commit, push,
and repeat forever.
...
1. Loads `AGENTS.md` from the repo root.
2. Prompts the local pi agent to read it and follow its loop **strictly**:
   pick the next mission feature from `roadmap.md` → implement across the Worldloop
   stack (`src/*.js` systems, `index.html`, `docs/adr/`) → run the browser smoke
   check / `check_world.mjs` until green → commit → push.
...
5. **Discards** the round's session and starts a brand-new one (new session id +
   fresh session log file) for the next round.
...
`SessionManager` (which runs `newSession()`, producing a new session id and a new
`<agentDir>/sessions/*.jsonl` log), prompts, then **disposes** that session once the
round completes. The previous round's session context and persisted log are fully
discarded, so no context or history carries across rounds.
...
- **If there IS committed progress and `lastCommit` matches `HEAD`** with a clean
  tree → pi-loop **RESUMES at the next round** (continues with current progress).
...
On startup it writes/merges this into `<agentDir>/models.json` and creates the SDK
session with `model: "deepseek-v4-flash"`.
...
- `AGENTS.md` and `roadmap.md` live at the **repo root** — pi-loop reads them each
  round so the agent always works from the latest plan.
- Progress is tracked by git commit, so every Worldloop feature must land as its
  own commit (see `AGENTS.md` rules) for resume-on-stop to be reliable.
